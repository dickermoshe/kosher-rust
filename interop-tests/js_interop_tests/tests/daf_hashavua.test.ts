  ...
  test("matches @hebcal/learning implementation for random dates 1900-2099", () => {
    for (let i = 0; i < ITERATIONS; i++) {
      ...
      if (!wasmResult || !hebcalResult) {
        ...
        console.log(
          `  WASM: ${wasmResult ? `${wasmResult.tractate}:${wasmResult.page}` : "null"}`,
        );
        console.log(
          `  Hebcal: ${hebcalResult ? `${hebcalResult.getName()}:${hebcalResult.getBlatt()}` : "null"}`,
        );
      }
