  ...
  test("matches @hebcal/learning implementation for random dates 1900-2099", () => {
    for (let i = 0; i < 200; i++) {
      ...
      if (!wasmResult || !hebcalResult) {
        ...
        console.log(
          `  Hebcal: ${hebcalResult ? JSON.stringify(hebcalResult) : "null"}`,
        );
      }
      ...
      if (wasmResult!.type === "Psalms" && hebcalParsed.type === "Psalms") {
        if (
          wasmResult!.start !== hebcalParsed.start ||
          wasmResult!.end !== hebcalParsed.end
        ) {
          const dateStr = `${date.getFullYear()}-${(date.getMonth() + 1).toString().padStart(2, "0")}-${date.getDate().toString().padStart(2, "0")}`;
        ...
        wasmResult!.type === "PsalmVerses" &&
        hebcalParsed.type === "PsalmVerses"
      ) {
        if (
          wasmResult!.psalm !== hebcalParsed.psalm ||
          ...
          console.log(
            `  WASM: ${wasmResult!.psalm}:${wasmResult!.start_verse}-${wasmResult!.end_verse}`,
          );
          console.log(
            `  Hebcal: ${hebcalParsed.psalm}:${hebcalParsed.start_verse}-${hebcalParsed.end_verse}`,
          );
        }
