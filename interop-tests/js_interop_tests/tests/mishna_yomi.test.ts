  ...
  test("matches @hebcal/learning implementation for random dates 1947-2099", () => {
    ...
    for (let i = 0; i < ITERATIONS; i++) {
      ...
      if (!wasmResult || !hebcalResult || hebcalResult.length !== 2) {
        ...
        console.log(
          `  WASM: ${wasmResult ? `${wasmResult.start.tractate} ${wasmResult.start.chapter}:${wasmResult.start.mishna} - ${wasmResult.end.tractate} ${wasmResult.end.chapter}:${wasmResult.end.mishna}` : "null"}`,
        );
        console.log(
          `  Hebcal: ${hebcalResult ? hebcalResult.map((m: any) => `${m.k} ${m.v}`).join(" - ") : "null"}`,
        );
      }
      ...
      if (
        wasmStartMapped !== hebcalStartMapped ||
        wasmEndMapped !== hebcalEndMapped
      ) {
        const dateStr = `${date.getFullYear()}-${(date.getMonth() + 1).toString().padStart(2, "0")}-${date.getDate().toString().padStart(2, "0")}`;
