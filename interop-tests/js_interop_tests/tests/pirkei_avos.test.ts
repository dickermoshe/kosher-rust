...
function hebcalToOurFormat(
  hebcalResult: number[] | null,
): PirkeiAvosResult | null {
  ...
  test("matches @hebcal/learning implementation for random Shabbat dates 1900-2099 (diaspora)", () => {
    for (let i = 0; i < ITERATIONS; i++) {
      ...
      if (!wasmResult || !hebcalMapped) {
        ...
        console.log(
          `  Hebcal: ${hebcalMapped ? JSON.stringify(hebcalMapped) : "null"}`,
        );
      }
      ...
      if (wasmResult!.type === "Single" && hebcalMapped!.type === "Single") {
        ...
        wasmResult!.type === "Combined" &&
        hebcalMapped!.type === "Combined"
      ) {
        if (
          wasmResult!.perek1 !== hebcalMapped!.perek1 ||
          wasmResult!.perek2 !== hebcalMapped!.perek2
        ) {
          const dateStr = `${date.getFullYear()}-${(date.getMonth() + 1).toString().padStart(2, "0")}-${date.getDate().toString().padStart(2, "0")}`;
          const hdate = new HDate(date);
          console.log(
          ...
          console.log(
            `  Hebcal: ${hebcalMapped!.perek1}-${hebcalMapped!.perek2}`,
          );
        }
