...
use super::{
    assertions::{
        assert_calendar_snapshots_match, assert_date_results_match, assert_jewish_date_snapshots_match,
    ...
    java_reference::{
        ...
        java_add_years_to_jewish_date_with_adar, java_gregorian_date_to_jewish_date,
        java_jewish_date_to_gregorian_date, java_minus_days_to_jewish_date,
    },
    policy,
    rust_reference::{
        rust_add_days_to_jewish_date, rust_add_months_to_jewish_date, rust_add_years_to_jewish_date_with_adar,
        rust_gregorian_date_to_jewish_date, rust_hebrew_date, rust_jewish_date_to_gregorian_date,
    },
    types::DateTuple,
};

pub(super) fn test_random_gregorian_to_jewish_date(rng: &mut StdRng, context: &str) -> Result<(), Box<dyn Error>> {
...
pub(super) fn test_random_jewish_to_gregorian_date(rng: &mut StdRng, context: &str) -> Result<(), Box<dyn Error>> {
    let date = random_valid_jewish_date(rng);
    assert_date_results_match(
        date,
        "Gregorian",
...
pub(super) fn test_random_jewish_date_snapshot(rng: &mut StdRng, context: &str) -> Result<(), Box<dyn Error>> {
    let date = random_valid_jewish_date(rng);
    assert_jewish_date_snapshots_match(date, context)?;
    Ok(())
}

pub(super) fn test_random_jewish_year_snapshot(rng: &mut StdRng, context: &str) -> Result<(), Box<dyn Error>> {
...
pub(super) fn test_random_add_days_to_jewish_date(rng: &mut StdRng, context: &str) -> Result<(), Box<dyn Error>> {
    let date = random_valid_jewish_date(rng);
...
pub(super) fn test_random_minus_days_to_jewish_date(rng: &mut StdRng, context: &str) -> Result<(), Box<dyn Error>> {
    let date = random_valid_jewish_date(rng);
...
pub(super) fn test_random_add_months_to_jewish_date(rng: &mut StdRng, context: &str) -> Result<(), Box<dyn Error>> {
    let date = random_valid_jewish_date(rng);
...
pub(super) fn test_random_add_years_to_jewish_date(rng: &mut StdRng, context: &str) -> Result<(), Box<dyn Error>> {
    let date = random_valid_jewish_date(rng);
...
pub(super) fn test_random_jewish_calendar(rng: &mut StdRng, context: &str) -> Result<(), Box<dyn Error>> {
    let date = random_valid_jewish_date(rng);
    assert_date_results_match(
        date,
        "Gregorian",
...
pub(super) fn test_random_invalid_jewish_date(rng: &mut StdRng, context: &str) -> Result<(), Box<dyn Error>> {
    let date = random_invalid_jewish_date(rng);
    assert_date_results_match(
        date,
        "Gregorian",
        java_jewish_date_to_gregorian_date(date)?,
...
fn random_gregorian_date(rng: &mut StdRng) -> DateTuple {
    let (min_gregorian_year, max_gregorian_year) = (policy::MIN_GREGORIAN_YEAR, policy::MAX_GREGORIAN_YEAR);
...
fn random_valid_jewish_date(rng: &mut StdRng) -> DateTuple {
    loop {
        let date = random_jewish_date(rng);
        if rust_hebrew_date(date).is_some() {
            return date;
        }
    }
}

fn random_invalid_jewish_date(rng: &mut StdRng) -> DateTuple {
    loop {
        let date = random_jewish_date(rng);
