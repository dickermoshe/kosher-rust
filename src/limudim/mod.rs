...
pub trait LimudCalendar {
    ...
    fn limud<U>(&self, limud_calculator: impl LimudCalculator<U>) -> Option<U>;
}
...
{
    fn limud<U>(&self, limud_calculator: impl LimudCalculator<U>) -> Option<U> {
        limud_calculator.limud(self.hebrew_date())
    }
}
// // Calculators
...
pub mod prelude {
    pub use super::{
        ...
        DafHashavuaBavli, DafYomiBavli, DafYomiYerushalmiVilna, LimudCalculator, LimudCalendar,
        MISHNA_YOMIS_CYCLE_DAYS, Mishna, MishnaYomis, Mishnas, PirkeiAvos, PirkeiAvosUnit, SHEKALIM_EXPANSION_CYCLE,
        Side, TehillimMonthly, TehillimUnit, Tractate, YERUSHALMI_DAF_COUNT,
    };
}
...
///
/// Returns `None` when `start > end`, or when ICU cannot compute the difference
/// (e.g. dates outside the supported Hebrew calendar range).
pub(crate) fn days_between(start: &Date<Hebrew>, end: &Date<Hebrew>) -> Option<u32> {
    if start > end {
        return None;
    }
    match start.try_until_with_options(end, DateDifferenceOptions::default()) {
