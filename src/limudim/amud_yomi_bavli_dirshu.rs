...
fn initial_cycle_date() -> Date<Hebrew> {
    ...
    let date = Date::try_new_gregorian(2023, 10, 16)
        .expect("hard-coded Gregorian date should be valid")
        .to_calendar(Hebrew);
    date
}

pub const fn start_daf(tractate: Tractate) -> Amud {
    match tractate {
...
pub const fn end_daf(tractate: Tractate) -> Option<Amud> {
    ...
    if amud.page == 0 { None } else { Some(amud) }
}
...
mod tests {
    ...
    fn amud_yomi_bavli_2025_23_9() {
        let test_date = from_gregorian_date(2025, 9, 23);
        let limud = AmudYomiBavliDirshu::default().limud(test_date).expect("limud exists");
        assert_eq!(limud.page, 34);
        assert_eq!(limud.side, Side::Aleph);
