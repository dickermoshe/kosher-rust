...
fn initial_cycle_date() -> Date<Hebrew> {
    ...
    let date = Date::try_new_gregorian(1980, 2, 2)
...
impl DafYomiYerushalmiVilna {
    fn cycle_start_date() -> Date<Hebrew> {
        initial_cycle_date()
    }
}
...
fn daf_yomi_yerushalmi_for_date(limud_date: Date<Hebrew>) -> Option<Daf> {
    let cycle_start = DafYomiYerushalmiVilna::cycle_start_date();
    ...
    let mut next_cycle = cycle_end_date(prev_cycle)?.add_days(1)?;
...
fn cycle_end_date(cycle_start: Date<Hebrew>) -> Option<Date<Hebrew>> {
    ...
    while found_days > 0 {
        ...
        end_date = end_date.add_days(found_days as i32)?;
        found_days = count_special_days(new_start_date, end_date)?;
    }
    Some(end_date)
}

fn tisha_bav_date(year: i32) -> Option<Date<Hebrew>> {
    let date = Date::try_new_hebrew_v2(year, month::AV, 9).ok()?;
...
fn count_special_days(start: Date<Hebrew>, end: Date<Hebrew>) -> Option<u32> {
    let start_year = start.year().extended_year();
    let end_year = end.year().extended_year();
    let mut special_days = 0;

    for year in start_year..=end_year {
        let yom_kippur = Date::try_new_hebrew_v2(year, month::TISHREI, 10).ok()?;
        if start < yom_kippur && yom_kippur <= end {
            special_days += 1;
        }

        let tisha_bav = tisha_bav_date(year);
        if start < tisha_bav? && tisha_bav? <= end {
            special_days += 1;
        }
    }
...
mod tests {
    ...
    use crate::{calendar::month::TISHREI, limudim::from_gregorian_date};

    use super::*;
    use crate::limudim::Tractate;
    ...
    fn daf_yomi_yerushalmi_last_skip_day() {
        ...
        let test_date = Date::<Hebrew>::try_new_hebrew_v2(5778, TISHREI, 10).expect("valid hebrew date");
        let limud = DafYomiYerushalmiVilna::default().limud(test_date);
