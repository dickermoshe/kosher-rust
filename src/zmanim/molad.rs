...
use crate::calendar::{chalakim_since_molad_tohu, prelude::*};
...
pub(crate) trait MoladCalendar {
    ...
    ///
    /// # Returns
    /// - `Some((DateTime, Month))` - The time representing 15 days after the molad and the Hebrew month
    /// - `None` - If the zman will not occur on this day
    //
    fn sof_zman_kidush_levana_15_days(&self, tz: &TimeZone) -> Option<(Timestamp, Month)>;
    ...
    ///
    /// # Returns
    /// - `Some((DateTime, Month))` - The time representing 3 days after the molad and the Hebrew month
    /// - `None` - If the zman will not occur on this day
    fn tchilas_zman_kidush_levana_3_days(&self, tz: &TimeZone) -> Option<(Timestamp, Month)>;
    ...
    ///
    /// # Returns
    /// - `Some((DateTime, Month))` - The time representing 7 days after the molad and the Hebrew month
    /// - `None` - If the zman will not occur on this day
    fn tchilas_zman_kidush_levana_7_days(&self, tz: &TimeZone) -> Option<(Timestamp, Month)>;
    ...
    ///
    /// # Returns
    /// - `Some((DateTime, Month))` - The time representing halfway between molad and molad and the Hebrew month
    /// - `None` - If the zman will not occur on this day
    fn sof_zman_kidush_levana_between_moldos(&self, tz: &TimeZone) -> Option<(Timestamp, Month)>;
    ...
    ///
    /// # Returns
    /// - `Some((DateTime, Month))` - The time of the molad and the Hebrew month
    /// - `None` - If the zman will not occur on this day
    fn molad(&self, tz: &TimeZone) -> Option<(Timestamp, Month)>;
}
    ...
    T: HebrewCalendarDate,
{
    fn sof_zman_kidush_levana_15_days(&self, tz: &TimeZone) -> Option<(Timestamp, Month)> {
        ...
        if is_same_gregorian_day(&hebrew, &molad, tz) {
            return Some((molad, hebrew.input_month()));
        }
        None
    }
    ...
    fn tchilas_zman_kidush_levana_3_days(&self, tz: &TimeZone) -> Option<(Timestamp, Month)> {
        ...
        if hebrew.day_of_month().0 == 30 {
            ...
            if is_same_gregorian_day(&hebrew, &molad, tz) {
                return Some((molad, new.input_month()));
            }
        }
        None
    }
...
fn abs_date_to_gregorian_date(abs_date: i64) -> Option<Date<Gregorian>> {
    ...
    let mut month: u8 = 1;
    while abs_date > gregorian_date_to_abs_date(year, month, get_last_day_of_gregorian_month(month, year)?)? {
        month += 1;
    }
