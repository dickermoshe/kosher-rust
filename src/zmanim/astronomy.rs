...
pub(crate) fn time_at_azimuth(date: Date, location: &Location, target_azimuth: f64) -> Result<Timestamp, ZmanimError> {
    ...
    instant_from_utc_hours(date_time * 24.0, adjusted_date, location, SolarEvent::None)
}
...
fn instant_from_utc_hours(
    ...
    match event {
        ...
        SolarEvent::None if local_time_hours > 18.0 => date = add_days(date, -1)?,
        SolarEvent::None if local_time_hours < 6.0 => date = add_days(date, 1)?,
        _ => {}
    }
...
fn normalize_utc_hours(time: f64) -> f64 {
    if time > 0.0 { time % 24.0 } else { time % 24.0 + 24.0 }
}
