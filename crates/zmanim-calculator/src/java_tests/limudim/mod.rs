...
use icu_calendar::{cal::Hebrew, Date as IcuDate, Gregorian};
use jiff::civil::Date;
use jni::{jni_sig, jni_str, objects::JString};
use limudim_calendar::{Daf, DafYomiBavli, DafYomiYerushalmiVilna, LimudCalendar, Tractate};
use rand::{rngs::StdRng, RngExt, SeedableRng};

use crate::{
    java_bindings::com::kosherjava::zmanim::hebrewcalendar::{
        Daf as JavaDaf, JewishCalendar, YerushalmiYomiCalculator, YomiCalculator,
    },
    java_tests::zmanim::java_reference::{init_bindings, java_vm, new_local_date},
};
...
fn test_iterations() -> u64 {
    match env::var("ZMANIM_LIMUDIM_JAVA_PARITY_ITERATIONS") {
        Ok(value) => value.parse::<u64>().unwrap_or_else(|_| {
            panic!("invalid ZMANIM_LIMUDIM_JAVA_PARITY_ITERATIONS value: {value}")
        }),
...
fn assert_yerushalmi_matches(
    date: Date,
    random_case: Option<(u64, u64)>,
...
fn hebrew_date(date: Date) -> IcuDate<Hebrew> {
    IcuDate::<Gregorian>::try_new_gregorian(
        i32::from(date.year()),
        date.month() as u8,
        date.day() as u8,
    )
...
fn java_jewish_calendar<'local>(
    env: &mut jni::Env<'local>,
    date: Date,
) -> jni::errors::Result<JewishCalendar<'local>> {
...
fn java_daf_to_rust(
    ...
    daf: &JavaDaf<'_>,
    talmud: Talmud,
) -> Result<Daf, Box<dyn Error>> {
    let page = env
        .call_method(daf, jni_str!("getDaf"), jni_sig!("()I"), &[])?
        .i()? as u16;
