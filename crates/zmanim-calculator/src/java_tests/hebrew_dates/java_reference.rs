...
use hebrew_holiday_calendar::Holiday;
...
use crate::{
    ...
    java_tests::zmanim::java_reference::{
        bool_to_jboolean, init_bindings, java_vm, new_local_date,
    },
};

use super::types::{DateTuple, JewishCalendarSnapshot, JewishDateSnapshot, JewishYearSnapshot};

pub(super) fn java_gregorian_date_to_jewish_date(
    date: DateTuple,
...
pub(super) fn java_jewish_date_to_gregorian_date(
    date: DateTuple,
) -> Result<Option<DateTuple>, Box<dyn Error>> {
    with_java_date_result(|env| {
...
pub(super) fn java_jewish_date_snapshot(
    date: DateTuple,
...
pub(super) fn java_jewish_year_snapshot(
    year: i32,
) -> Result<Option<JewishYearSnapshot>, Box<dyn Error>> {
    with_java_result(|env| {
